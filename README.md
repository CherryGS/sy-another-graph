...
- Search, notebook/relation filters, visible-graph isolation filtering, node
  details, and document navigation.
...
The host adapter is CommonJS. Its same-origin iframe contains the ESM workbench;
TanStack Router uses iframe-local hash history and does not change SiYuan's
application route. The bridge only accepts document-opening messages from its
own iframe and origin.
