...
import { projectGraph, numericTopology } from "../../core/scope/graph-model";
import { type GraphDataset, type GraphEdge, type GraphNode } from "../../core/graph/types";
...
describe("mentions inside projected native scope", () => {
  it("keeps native nodes and references when their names are excluded from text mentions", () => {
    ...
    expect(projectGraph(source, { ...DEFAULT_FILTERS, excludedMentionPhrases: ["01"] })).toEqual(
      graph,
    );
  ...
  it("does not treat a display-only owning document as an eligible source or target", () => {
    const source = data([
      node(0, "a"),
      node(1, "section", "h", "a", "a"),
      node(2, "p", "p", "a", "section"),
      node(3, "outside", "p", "a", "a"),
    ]);
  ...
  it("adds mention endpoints to the same numeric topology used for neighborhoods and paths, and removes them on off", () => {
    ...
    expect(enabled.nodes.map((node) => graphDegrees(enabled).get(node.index) ?? 0)).toEqual([1, 1]);
    ...
    expect(base.nodes.map((node) => graphDegrees(base).get(node.index) ?? 0)).toEqual([0, 0]);
  });
});
