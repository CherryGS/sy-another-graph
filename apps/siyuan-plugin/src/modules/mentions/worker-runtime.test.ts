...
const query = (
  ...
  chosenIds: string[] = [],
): MentionRequest => ({ kind: "query", revision: 1, scopeRevision: 1, request, mode, chosenIds });
const results = (events: MentionResponse[]) =>
  events.filter(
    (event): event is Extract<MentionResponse, { kind: "result" }> => event.kind === "result",
  );
...
describe("mention worker scheduling", () => {
  ...
  it("rebuilds exclusions using the retained corpus and publishes only the latest revision", async () => {
    ...
    expect(
      events.some(
        (event) => event.revision === 2 && (event.kind === "ready" || event.kind === "result"),
      ),
    ).toBe(false);
    runtime.dispose();
  });
  ...
  it("honors off during indexing and rejects results for superseded corpus/scope requests", async () => {
    ...
    expect(
      results(events).every((event) => event.request === 2 && event.result.edges.length === 0),
    ).toBe(true);
    runtime.receive({
      kind: "load",
      revision: 2,
      blocks: blocks.map((block) =>
        block.id === "p" ? { ...block, markdown: "No match" } : block,
      ),
    });
    runtime.receive({ kind: "scope", revision: 2, scopeRevision: 1, scope });
    runtime.receive({ ...query(3, "all"), revision: 2 });
    await vi.waitFor(() =>
      expect(results(events).some((event) => event.revision === 2)).toBe(true),
    );
...
class WorkerStub extends EventTarget implements MentionWorkerPort {
  ...
  terminate = vi.fn();
  postMessage(message: MentionRequest) {
    this.messages.push(message);
  }
  emit(message: MentionResponse) {
...
describe("mention client revision boundaries", () => {
  ...
  it("does not reload text for mode/selection changes and rejects late replies at every boundary", () => {
    ...
    worker.emit({
      kind: "ready",
      revision: 1,
      progress: { ...EMPTY_MENTION_PROGRESS, scanned: 1, total: 1 },
    });
