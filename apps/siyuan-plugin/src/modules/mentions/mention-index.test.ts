...
const doc = (id: string, title: string, ial = ""): MentionBlock => ({
  id,
  rootId: id,
  type: "d",
  title,
...
const paragraph = (id: string, rootId: string, markdown: string, ial = ""): MentionBlock => ({
  id,
  rootId,
  type: "p",
  title: "",
...
const scopeOf = (blocks: MentionBlock[]): MentionScope => ({
...
describe("ordinary source prose and vocabulary", () => {
  it("keeps formatted prose but excludes native refs, Markdown links, code, URLs, attributes, and raw HTML", () => {
    const prose = ordinaryProse(
      'Graph **Theory** and art. `InlineSecret` [LinkSecret](https://example.test) ((20260822181032-6spbotb "NativeSecret")) https://example.test/UrlSecret {: name="AttributeSecret"}\n\n```ts\nCodeSecret\n```\n\n<span title="AttributeSecret">HtmlSecret</span> VisibleAgain',
    );
  ...
  it("reads native names and aliases without mistaking custom attributes for names", () => {
    expect(
      nativeNames(
        "Document",
        '{: custom-name="Ignore" name="A&amp;B" alias="Other,With\\,Comma,&#x56;alue"}',
      ),
  ...
  it("matches whole Latin words and embedded Chinese names with original Unicode offsets", () => {
    ...
    expect(matcher.scan(text, 100).hits.map((hit) => text.slice(hit.start, hit.end))).toEqual([
      "art",
      "ART",
      "图论",
      "ＡＲＴ",
...
describe("cached text mention graph", () => {
  ...
  it("excludes complete normalized names in both modes without excluding larger numeric names", async () => {
    const blocks = [
      ...
      doc("noise", "01"),
      doc("number", "101"),
      doc("phrase", "Graph Theory"),
      paragraph("p", "source", "01 101 Graph Theory"),
    ];
  ...
  it("excludes a noisy alias while retaining the same node's other names", async () => {
    const blocks = [
      doc("source", "Source"),
      doc("target", "Useful", '{: name="01" alias="Noise,Helpful"}'),
    ...
    expect(result.edges[0].provenance!.map((item) => item.mention!.matched)).toEqual([
      "Helpful",
      "Useful",
    ]);
  });
  ...
  it("frees vocabulary and occurrence budgets before noisy words can crowd out useful matches", async () => {
    const blocks = [
      doc("noise", "01"),
      doc("useful", "Beta"),
      paragraph("p", "source", `${"01 ".repeat(100)}Beta`),
    ];
    const index = await indexOf(
      blocks,
      { keywords: 1, occurrencesPerSource: 1, cachedOccurrences: 1 },
      ["01"],
    );
    const result = await index.query(scopeOf(blocks), "all", [], controller().signal);
    expect(result.edges).toHaveLength(1);
    expect(result.edges[0].provenance![0]).toMatchObject({
      sourceId: "p",
      targetId: "useful",
      mention: { matched: "Beta" },
    });
    expect(index.progress).toMatchObject({ keywords: 1, skippedKeywords: 0, limitedSources: 0 });
    expect(result.truncated).toBe(false);
  });
  ...
  it("recomputes cached matches when exclusions change and restores them when cleared", async () => {
    const blocks = [
      doc("source", "Source"),
      doc("short", "Graph"),
      doc("long", "Graph Theory"),
      paragraph("p", "source", "Graph Theory"),
    ];
    const index = await indexOf(blocks);
  ...
  it("keeps source paragraphs and counts occurrences, with explicit edges taking precedence", async () => {
    const blocks = [
      doc("a", "Alpha"),
      doc("b", "Beta"),
      paragraph("p", "a", "Beta and **Beta**. Alpha ((20260822181032-6spbotb 'Beta'))"),
    ];
    const index = await indexOf(blocks);
    const scope = scopeOf(blocks);
    const all = await index.query(scope, "all", [], controller().signal);
    expect(all.edges).toHaveLength(1);
    expect(all.edges[0]).toMatchObject({
      source: 2,
      target: 1,
      kind: "text-mention",
      weight: 2,
      provenance: [
        { sourceId: "p", targetId: "b", weight: 2, mention: { matched: "Beta", candidates: 1 } },
      ],
    });
    const evidence = all.edges[0].provenance![0].mention!;
    expect(evidence.excerpt.slice(evidence.start, evidence.end)).toBe("Beta");
    expect(
      (await index.query({ ...scope, explicitPairs: [[2, 1]] }, "all", [], controller().signal))
        .edges,
    ).toEqual([]);
  });
  ...
  it("supports selected documents' own prose, incoming mentions, multi-selection union, and off", async () => {
    const blocks = [
      ...
      doc("c", "Gamma"),
      paragraph("pa", "a", "Beta"),
      paragraph("pc", "c", "Alpha"),
    ];
    const index = await indexOf(blocks);
    const scope = scopeOf(blocks);
    const all = await index.query(scope, "all", [], controller().signal);
    expect(all.edges).toHaveLength(2);
    expect((await index.query(scope, "selected", ["a"], controller().signal)).edges).toEqual(
      all.edges,
    );
    expect((await index.query(scope, "selected", ["b", "c"], controller().signal)).edges).toEqual(
      all.edges,
    );
    expect((await index.query(scope, "selected", ["b"], controller().signal)).edges).toHaveLength(
      1,
    );
  ...
  it("filters both source and target before resolving ambiguous and overlapping names", async () => {
    ...
    const smaller = {
      ...scope,
      entries: scope.entries.filter((entry) => !["c", "d"].includes(entry.id)),
    };
    ...
    expect(
      (
        await index.query(
          { ...scope, entries: scope.entries.filter((entry) => entry.id !== "p") },
          "all",
          [],
          controller().signal,
        )
  ...
  it("aggregates hidden paragraphs into document endpoints while retaining native evidence", async () => {
    const blocks = [
      ...
      doc("b", "Beta"),
      paragraph("p1", "a", "Beta"),
      paragraph("p2", "a", "Beta Beta"),
    ];
    const index = await indexOf(blocks);
    const scope = scopeOf(blocks);
    scope.entries = scope.entries.map((entry) =>
      entry.id.startsWith("p") ? { ...entry, index: 0, displayId: "a" } : entry,
    );
  ...
  it("reuses unchanged matches, invalidates edited text and names, and drops deleted targets", async () => {
    ...
    expect(index.progress.cached).toBe(0);
    expect(
      (await index.query(scopeOf(renamed), "all", [], controller().signal)).edges[0].provenance![0]
        .mention!.matched,
    ).toBe("Gamma");
  ...
  it("does not scan containers/code or create virtual document self-loops", async () => {
    const blocks = [
      doc("a", "Alpha"),
      doc("b", "Beta"),
      { ...paragraph("list", "a", "Beta"), type: "l" },
      { ...paragraph("code", "a", "Beta"), type: "c" },
      paragraph("p", "a", "Alpha"),
    ];
    const index = await indexOf(blocks);
  ...
  it("uses exact case for single-character names while keeping their word boundaries", async () => {
    ...
    const index = await indexOf(blocks);
    expect(
      (await index.query(scopeOf(blocks), "all", [], controller().signal)).edges[0].weight,
    ).toBe(1);
  });
  ...
  it("reports bounded acquisition/index/display evidence instead of claiming complete results", async () => {
    const blocks = [
      ...
      doc("b", "Beta"),
      doc("c", "Gamma"),
      paragraph("p", "a", "Beta Gamma"),
      paragraph("q", "a", "Beta"),
      paragraph("long", "a", "x".repeat(60)),
    ];
    ...
    expect((await index.query(scopeOf(blocks), "all", [], controller().signal)).truncated).toBe(
      true,
    );
    const scope = scopeOf(blocks);
    scope.entries = scope.entries
      .map((entry) =>
        ["p", "q"].includes(entry.id) ? { ...entry, index: 0, displayId: "a" } : entry,
      )
  ...
  it("cancels obsolete warm/query jobs", async () => {
    ...
    await expect(index.query(scopeOf(blocks), "all", [], abort.signal)).rejects.toMatchObject({
      name: "AbortError",
    });
  });
  ...
  it("finishes cheap multi-batch queries without waiting for timers", async () => {
    ...
    try {
      expect(await index.query(scopeOf(blocks), "selected", ["d0"], controller().signal)).toEqual({
        edges: [],
        truncated: false,
        ambiguousEdges: 0,
      });
  ...
  it("lets queued cancellation interrupt a query after its work budget is spent", async () => {
    ...
    try {
      ...
      const rejected = expect(
        index.query(scopeOf(blocks), "all", [], abort.signal),
  ...
  it("keeps shared limited passages stable and enforces the cache budget after source reordering", async () => {
    const blocks = [
      doc("a", "Source"),
      doc("b", "Beta"),
      paragraph("p", "a", "Beta Beta"),
      paragraph("q", "a", "Beta Beta"),
    ];
    ...
    expect(index.progress.limitedSources).toBe(2);
    const cached = await indexOf([blocks[0], blocks[1], paragraph("old", "a", "Beta")], {
      cachedOccurrences: 1,
    });
    const extended = [
      blocks[0],
      blocks[1],
      paragraph("new", "a", "Beta!"),
      paragraph("old", "a", "Beta"),
    ];
    cached.replace(extended);
    await cached.warm(() => {}, controller().signal);
    expect(
      (await cached.query(scopeOf(extended), "all", [], controller().signal)).edges,
    ).toHaveLength(1);
  ...
  it("deduplicates a native target's normalized names without collapsing other native candidates", async () => {
    const blocks = [
      doc("a", "Source"),
      doc("b", "Beta", '{: name="BETA" alias="Beta,beta"}'),
      doc("c", "Beta"),
      paragraph("p", "a", "Beta"),
    ];
    const index = await indexOf(blocks);
