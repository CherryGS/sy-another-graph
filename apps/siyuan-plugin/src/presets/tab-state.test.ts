...
import { DEFAULT_FILTERS, type GraphDataset } from "../data/types";
import { GraphTabTitle } from "../host/graph-tab-title";
...
const data: GraphDataset = {
  source: "siyuan", loadedAt: "", loadMs: 0, referenceCount: 0, skippedReferences: 0, warnings: [],
  notebooks: [{ id: BOOK, name: "Research", color: "#fff" }], edges: [],
  nodes: [{ id: BLOCK, index: 0, label: "Project A", notebook: BOOK, blockType: "d", path: "", color: "#fff", degree: 0 }],
};
...
describe("semantic graph tab state", () => {
  ...
  it("ignores search, node counts, degrees, and acquisition timings but tracks source name changes", () => {
    const filters = { ...DEFAULT_FILTERS, scopeId: BLOCK };
    const first = graphTabState(filters, data, "Reading", false);
    const refreshed = { ...data, loadedAt: "later", loadMs: 10_000, referenceCount: 999,
      nodes: [{ ...data.nodes[0], degree: 77 }, { ...data.nodes[0], id: "other", index: 1 }] };
    expect(graphTabState({ ...filters, query: "a new search" }, refreshed, "Reading", false)).toEqual(first);
    expect(graphTabState(filters, { ...refreshed, nodes: [{ ...data.nodes[0], label: "Renamed" }] }, "Reading", false).title).toBe("图谱 · Renamed · Reading");
  });
  ...
  it("removes forbidden controls and truncates long Unicode without splitting code points or violating host limits", () => {
    ...
    const large = { ...data, nodes: [{ ...data.nodes[0], label: title }], notebooks: [{ ...data.notebooks[0], name: title }] };
    const filters = { ...DEFAULT_FILTERS, scopeId: BLOCK, notebook: BOOK, documentsOnly: false,
    ...
    for (const value of [state.title, state.description]) {
      expect(Array.from(value).every(character => {
    ...
    expect(host.handle({ data: { channel: "sy-another-graph", type: "graph-tab-state", ...state } } as MessageEvent)).toBe(true);
    expect(host.title).toBe(state.title);
    const cleaned = graphTabState({ ...DEFAULT_FILTERS, scopeId: BLOCK }, { ...data, nodes: [{ ...data.nodes[0], label: "A\u0000\u0081\nB" }] }, "Read\u0000\u0081\nmode", false);
