...
function harness(initial: PresetStore | null = saved(), ready = true) {
  ...
  const port = {
    ...
    save: vi.fn(async (store: PresetStore): Promise<PresetStore | null> => {
      disk = structuredClone(store);
      return structuredClone(store);
    }),
    dispose: vi.fn(),
  };
  const publish = vi.fn((next: PresetSnapshot) => {
    snapshot = next;
  });
  ...
  return {
    ...
    setDisk(value: PresetStore | null) {
      disk = value;
    },
    ready() {
      sourceReady = true;
...
describe("preset persistence and drafts", () => {
  it("keeps drafts separate through update, copying the saved baseline, conservative creation, and save-as", async () => {
    ...
    expect(h.filters).toMatchObject({
      documentsOnly: true,
      hierarchy: false,
      mentions: "off",
      query: "keep search",
  ...
  it.each([null, { ...saved(), activePresetId: "reading" }])(
    "rejects absent or mismatched save acknowledgements",
    async (result) => {
      const h = harness();
      await h.controller.load();
      const before = h.snapshot.store;
...
describe("asynchronous preset activation", () => {
  it.each(["apply", "create", "copy"] as const)(
    "%s preserves query-only edits while a save is pending",
    async (action) => {
      const h = harness();
      await h.controller.load();
      ...
      const pending =
        action === "apply"
          ? h.controller.apply("reading")
          : action === "copy"
            ? h.controller.copy("reading", "Copy")
            : h.controller.create("Fresh");
  ...
  it.each([false, true])(
    "keeps newer explicit rules when a delayed activation completes, including reverted edits=%s",
    async (revert) => {
      const h = harness();
      await h.controller.load();
      const saving = deferred<PresetStore | null>();
  ...
  it.each(["notebook", "scopeId"] as const)(
    "rejects stale %s for initial restoration and copy just as for manual apply",
    async (field) => {
      const store = { ...saved(), activePresetId: "reading" };
  ...
  it.each(["pending read", "pending source"])(
    "never restores over explicit rules while %s, even after reverting them",
    async (stage) => {
      const store = { ...saved(), activePresetId: "reading" };
