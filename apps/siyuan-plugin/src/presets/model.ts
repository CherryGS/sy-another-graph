...
export interface PresetStore { version: 1; presets: FilterPreset[]; activePresetId: string | null }
...
const hasControl = (value: string) => Array.from(value).some(character => {
...
export function presetFilters(filters: PresetFilters): PresetFilters {
  return {
    ...
    hiddenTypes: filters.documentsOnly ? [] : uniqueSorted(filters.hiddenTypes.filter(type => type !== "d")),
...
export function applyPresetFilters(previous: GraphFilters, rules: PresetFilters): GraphFilters {
  ...
  const sameValues = (left: readonly string[], right: readonly string[]) => JSON.stringify(uniqueSorted(left)) === JSON.stringify(uniqueSorted(right));
...
export function createPresetStore(): PresetStore {
  return { version: 1, presets: [{ id: "documents", name: "文档引用", filters: presetFilters(DEFAULT_FILTERS) }], activePresetId: "documents" };
}

function readFilters(value: unknown): PresetFilters | null {
  if (!record(value)) return null;
  for (const key of ["references", "hierarchy", "hideIsolated", "includeChildDocuments", "databases", "documentsOnly"])
    if (typeof value[key] !== "boolean") return null;
  for (const key of ["notebook", "scopeId"])
    if (typeof value[key] !== "string" || (value[key] !== "" && !NATIVE_ID.test(value[key]))) return null;
  if (!isMentionMode(value.mentions)
    || !Array.isArray(value.excludeIds) || value.excludeIds.length > 2_000
...
export function readPresetStore(value: unknown): PresetStore | null {
  if (!record(value) || value.version !== 1 || !Array.isArray(value.presets) || value.presets.length > PRESET_LIMIT
    || (value.activePresetId !== null && typeof value.activePresetId !== "string")) return null;
  const presets: FilterPreset[] = [];
  const ids = new Set<string>();
  for (const item of value.presets) {
    if (!record(item) || typeof item.id !== "string" || !PRESET_ID.test(item.id) || ids.has(item.id)
      || typeof item.name !== "string") return null;
    const filters = readFilters(item.filters);
    if (!filters) return null;
    let name: string;
    try { name = presetName(item.name); } catch { return null; }
    ids.add(item.id);
