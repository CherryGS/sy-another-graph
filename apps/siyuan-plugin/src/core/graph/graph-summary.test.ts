...
function node(id: string, index: number, blockType = "p"): GraphNode {
  return { id, index, blockType, label: id, notebook: "book", path: "" };
}
...
describe("cached node type counts", () => {
  it("preserves first-seen native and logical type identities", () => {
    const nodes = Object.freeze([
      ...
      node("paragraph", 1),
      { ...node("database", 2, "av"), entity: "database" as const },
      node("second-paragraph", 3),
      { ...node("item", 4), entity: "database-item" as const },
      node("unknown", 5, "__proto__"),
      node("second-document", 6, "d"),
    ]);
  ...
  it("does not recount one published array after remount, and refreshes changed type facts", () => {
    ...
    const after = Object.freeze([node("source", 0, "h")]);
