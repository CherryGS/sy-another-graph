...
export function readReport(data: GraphDataset): string {
  const lines = [
    "一个思源图谱 — read diagnostics",
    `Read at: ${data.loadedAt}`,
    `Read time: ${data.loadMs.toFixed(1)} ms`,
    `Nodes: ${data.nodes.length}; relationships: ${data.edges.length}; source references: ${data.referenceCount}`,
    `Issue categories: ${data.warnings.length}`,
    "",
    "Details refer to the acquired workspace index, before graph display filters.",
  ];
  for (const issue of data.warnings) {
    ...
    issue.details.forEach((detail, index) => {
      lines.push(
        `  ${index + 1}.`,
        ...Object.entries(detail.fields).map(([label, value]) => `    ${label}: ${value}`),
      );
