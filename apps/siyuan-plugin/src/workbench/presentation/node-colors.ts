...
export function nodeTypeColor(type: string): string {
  ...
  for (let index = 0; index < type.length; index++)
    hash = Math.imul(hash ^ type.charCodeAt(index), 16777619);
...
export function nodeColor(node: CanvasNode, mode: GraphColorMode): string {
  ...
  if (mode === "degree") {
    ...
    const position = Math.min(1, Math.log2(degree + 1) / 8) * (DEGREE_COLORS.length - 1);
    const lower = Math.min(Math.floor(position), DEGREE_COLORS.length - 2);
    const fraction = position - lower;
    const rgb = DEGREE_COLORS[lower].map((channel, index) =>
      Math.round(channel + (DEGREE_COLORS[lower + 1][index] - channel) * fraction)
        .toString(16)
        .padStart(2, "0"),
    );
    return `#${rgb.join("")}`;
  ...
  const branch = segments[Math.min(1, segments.length - 1)].replace(/\.sy$/i, "");
