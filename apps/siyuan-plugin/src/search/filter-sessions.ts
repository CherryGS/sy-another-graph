...
export class FilterSessions {
  ...
  private state = { normal: this.normalRef.current, temporary: null as TemporarySearch | null, active: false };
  generation = 0;
  ...
  setFilters = (action: FilterAction) => {
    ...
    if (!temporary) { this.setNormal(action); return; }
  ...
  leave = () => { this.generation++; this.publish({ active: false }); };
  scope = (id: string) => { this.leave(); this.setNormal(previous => ({ ...previous, scopeId: id })); };
