...
export function unsupportedSearch(config: SearchConfig): string {
  ...
  if (!config.query?.trim() && !config.idPath?.length) return "请输入搜索条件；最近更新列表不属于搜索结果。";
  if (![0, 1, 2, 3].includes(config.method ?? 0)) return "暂不支持语义搜索建图，请使用关键字、查询语法、SQL 或正则搜索。";
  return "";
}
...
export async function collectSearchResults(
  ...
  if (config.method === 2) {
    ...
    await request("/api/query/sql", { stmt: readOnlySearchProbe(config.query ?? ""), mode: "readonly" }, signal);
  }
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    ...
    const result = await request<{
      ...
      query: config.query ?? "", method: config.method ?? 0,
      types: config.types, subTypes: config.subTypes, paths: config.idPath ?? [],
      groupBy: 0, orderBy: config.sort ?? 0, page, pageSize: PAGE_SIZE,
    ...
    if (total > MAX_SEARCH_RESULTS) throw new Error(`搜索命中 ${total.toLocaleString()} 个块，超过临时建图的 ${MAX_SEARCH_RESULTS.toLocaleString()} 个结果上限，请缩小搜索范围。`);
    if (expected !== undefined && total !== expected)
      throw new Error(`搜索结果在读取期间发生变化（${expected} → ${total}），请重新搜索后建图。`);
    expected = total;
    if (!total) throw new Error("没有可用于建图的搜索结果，请重新搜索。加密笔记本暂不支持搜索建图。");
    ...
    if (ids.size > total || ids.size === before)
      throw new Error(`搜索分页不完整：已读取 ${ids.size} / ${total} 个不同的块（第 ${page} 页）。请重新搜索；复杂 SQL 可能无法完整分页。`);
  ...
  throw new Error(`搜索分页超过 ${MAX_PAGES} 页，已读取 ${ids.size} / ${expected} 个块，请缩小范围或移除 SQL 中过小的 LIMIT。`);
}
