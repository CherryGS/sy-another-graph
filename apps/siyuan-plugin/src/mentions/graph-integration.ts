...
export function mentionScope(data: GraphDataset, graph: CurrentGraph): MentionScope {
  ...
  for (const id of graph.sourceIds) {
    ...
    if (source && isBlock(source) && node) entries.push({ id, displayId: node.id, index: node.index });
  }
  return { entries, explicitPairs: graph.edges.filter(edge => edge.kind === "reference").map(edge => [edge.source, edge.target]) };
}
...
export function withMentionEdges(base: CurrentGraph, mentions: readonly GraphEdge[]): CurrentGraph {
  ...
  const valid = mentions.filter(edge => edge.kind === "text-mention" && nodes.has(edge.source) && nodes.has(edge.target)
    && !!edge.provenance?.length && edge.provenance.every(item => base.sourceIds.has(item.sourceId) && base.sourceIds.has(item.targetId)));
  ...
  return { ...base, nodes: base.nodes.map(node => ({ ...node, degree: node.degree + (extra.get(node.index) ?? 0) })), edges: [...base.edges, ...valid] };
}
