...
> = {
  ...
  "reference-endpoints": {
    ...
    impact:
      "这些引用没有加入图谱。这里的缺失指本次工作空间块索引中找不到端点，与图谱显示筛选不同。",
    suggestion:
      "打开可用来源检查引用目标；确认目标是否已删除、所在笔记本是否打开，或索引是否尚未更新。详情只记录实际读取结果，不推断删除原因。",
  },
  "database-identifier": {
    title: "数据库载体无法识别",
    ...
    suggestion:
      "打开来源确认数据库载体能否正常显示，检查其 data-av-id；不要把载体块 ID 当作数据库 ID。",
  },
  "database-read": {
    title: "数据库未能完整读取",
    ...
    suggestion:
      "查看具体接口错误或首个校验失败的位置、实际值和期望格式。可打开数据库来源核实，重新读取或复制报告用于排查。",
  },
  "database-budget": {
    title: "数据库读取达到上限",
  ...
  "database-bindings": {
    ...
    impact:
      "真实条目和库内成员关系已保留，仅省略不可用的绑定连接。缺失指本次工作空间块索引，而非显示筛选。",
...
export class ReadIssueCollector {
    ...
    if (group.details.length < ReadIssueCollector.detailLimit)
      group.details.push({
        ...
        fields: Object.fromEntries(
          Object.entries(detail.fields).map(([key, value]) => [key, diagnosticValue(value)]),
        ),
      });
  }
  ...
  finish(): ReadIssue[] {
    return [...this.groups].map(([code, group]) => ({
      ...
      ...group,
      code,
      summary: definitions[code].summary(group.count),
    }));
  }
}
...
export function diagnosticValue(value: unknown): string {
  ...
  if (typeof value === "string")
    return value.length > 500 ? `${value.slice(0, 500)}…（已截断）` : value || "（空字符串）";
