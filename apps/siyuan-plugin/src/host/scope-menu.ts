...
export function documentTreeScopeId(
  detail: IEventBusMap["open-menu-doctree"],
): string | null {
...
export function registerScopeMenus(
  ...
  const addItem = (
    ...
    if (disposed || !id || menu.menus.some((item) => item.id === SCOPE_MENU_ID))
      return;
    menu.addItem({
      id: SCOPE_MENU_ID,
      icon: "iconAtlasGraph",
  ...
  const onDocumentMore = ({
    detail,
  }: CustomEvent<IEventBusMap["open-menu-breadcrumbmore"]>) =>
    addItem(detail.menu, documentScopeId(detail));
  const onDocumentTree = ({
    detail,
  }: CustomEvent<IEventBusMap["open-menu-doctree"]>) =>
