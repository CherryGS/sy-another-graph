...
import { collectSearchResults, unsupportedSearch, type SearchConfig } from "../search/collect";
import type { SearchGraphSnapshot } from "../search/model";
...
export function registerSearchGraphs(
  ...
  const update = (panel: SearchPanel) => {
    ...
    panel.button.title = panel.config ? unsupportedSearch(panel.config) || "将全部分页结果放入独立临时预设" : "等待搜索完成后建图";
  };
  const ensure = (input: HTMLInputElement) => {
    ...
    const toolbar = input.closest(".fn__flex-column")?.querySelector("#searchResult")?.parentElement;
    ...
    const panel: SearchPanel = { input, button, config: null, requestValue: null, abort: null, invalidate: () => {
      panel.abort?.abort(); panel.abort = null; panel.config = null; panel.requestValue = null; update(panel);
    } };
    ...
    button.addEventListener("click", event => {
      ...
      if (panel.abort) { panel.abort.abort(); panel.abort = null; update(panel); return; }
      if (!panel.config) return;
      const reason = unsupportedSearch(panel.config);
      if (reason) { report(reason); return; }
      const notebooks = (window as unknown as { siyuan?: { notebooks?: { id: string; encrypted?: boolean }[] } }).siyuan?.notebooks ?? [];
      if (panel.config.idPath?.some(path => notebooks.some(book => book.id === path.split("/")[0] && book.encrypted))) {
        report("暂不支持加密笔记本的搜索结果建图，请选择普通笔记本后重试。");
        return;
      }
      // A newer click in any search panel supersedes the previous request.
      for (const other of panels.values()) { other.abort?.abort(); other.abort = null; update(other); }
      const generation = ++latest;
      const abort = new AbortController();
      panel.abort = abort;
      const timeout = setTimeout(() => abort.abort(new Error("搜索读取超过 2 分钟，请缩小范围后重试。")), 120_000);
      ...
      void collectSearchResults(config, abort.signal, (read, total) => {
        if (!abort.signal.aborted) button.textContent = `${read.toLocaleString()} / ${total.toLocaleString()} · 取消`;
      }).then(async ids => {
        if (disposed || abort.signal.aborted || generation !== latest || !input.isConnected) return;
        ...
        if (!disposed && !abort.signal.aborted) input.closest(".b3-dialog")?.querySelector(".b3-dialog__close")?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
      }).catch((failure: unknown) => {
        if (disposed || generation !== latest) return;
        if (abort.signal.aborted && !(abort.signal.reason instanceof Error && abort.signal.reason.name !== "AbortError")) return;
        report(failure instanceof Error ? failure.message : String(failure));
      }).finally(() => {
        clearTimeout(timeout);
        if (panel.abort === abort) { panel.abort = null; update(panel); }
      });
    });
