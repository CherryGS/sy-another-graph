...
function harness() {
  ...
  const doc = {
    ...
    createElement: () => ({
      dataset: {} as Record<string, string>, style: {},
      setAttribute: vi.fn(), remove: vi.fn(),
      dispatchEvent() { eventTargets.push(this); },
    }),
    elementFromPoint: vi.fn(),
  };
  const frame = {
    ownerDocument: doc, clientWidth: 1000, clientHeight: 800,
  ...
  const enter = (overrides = {}) => preview.handle({
    channel: "sy-another-graph", type: "native-preview", action: "enter", token: 1,
    id: "20260822181032-6spbotb",
    rect: { left: 600, top: 100, width: 280, height: 70 }, ...overrides,
  });
  const leave = (overrides = {}) => preview.handle({
    channel: "sy-another-graph", type: "native-preview", action: "leave", token: 1,
    x: 650, y: 160, ...overrides,
  });
...
describe("host native block preview", () => {
  it("positions an inert reference at the title's scaled host viewport rectangle", () => {
    ...
    expect(anchor.style).toMatchObject({ left: "600px", top: "140px", width: "224px", height: "56px" });
  ...
  it("rejects invalid identities, channels, dimensions, and off-frame anchors", () => {
    ...
    for (const override of [
      { id: "av:unknown" }, { channel: "other" }, { token: "1" },
