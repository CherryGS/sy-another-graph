...
import {
  fillMissingPointPositions,
  getRandomPointPositions,
} from "@cosmograph/cosmograph/cosmograph/graph-utils/points.js";
...
function cpuGraph(deviceLimit?: number, dimensions: 2 | 3 = 2) {
  ...
  const graph = new Graph(
    {},
    { spaceSize: 8192, spaceDimensions: dimensions },
    new Promise(() => {}),
  );
  if (deviceLimit !== undefined) {
    ...
    graph.device = {
      ...
      get gl() {
        throw new Error("Simulation-space diagnostics must not access WebGL");
      },
    };
  }
  return graph;
...
describe("native simulation-space diagnostics", () => {
  it.each([
    ...
    "reports the actual native cap for a $deviceLimit device",
    ({ deviceLimit, effectiveSize }) => {
  ...
  it.each([2, 3] as const)(
    "returns independent CPU snapshots without changing a %sD view",
    (dimensions) => {
      const graph = cpuGraph(16384, dimensions);
      const forbidden = vi.fn(() => {
        throw new Error("Diagnostics must not change or read the rendered view");
      });
...
describe("public Cosmograph simulation-space bridge", () => {
  ...
  it("forwards the current native snapshot without deriving a cap from configuration", () => {
    ...
    expect(
      Reflect.apply(Cosmograph.prototype.getSimulationSpaceInfo, { _cosmos: source }, []),
    ).toBe(snapshot);
...
describe("effective simulation space for generated positions", () => {
  it.each([2, 3] as const)(
    "initializes %sD positions around the effective world center",
    async (dimensions) => {
      vi.spyOn(console, "warn").mockImplementation(() => {});
      const { context, cosmos } = seedContext(dimensions);
      const updated = await Reflect.apply(
        Reflect.get(CosmographConfigManager.prototype, "_handlePointPositions"),
        context,
        [undefined, { numRows: 8 }],
      );
      ...
      expect(positions).toEqual(
        Float32Array.from(getRandomPointPositions(8, 4096, seed, dimensions)),
      );
  ...
  it("preserves finite XY coordinates while initializing Z inside the effective world", async () => {
    ...
    await Reflect.apply(
      Reflect.get(CosmographConfigManager.prototype, "_handlePointPositions"),
      context,
      [{ spaceDimensions: 2 }, { numRows: 2 }],
    );
    const expected = Float32Array.from(getRandomPointPositions(2, 4096, seed, 3));
    expected[0] = 125;
    expected[1] = 375;
    expected[4] = 700;
    expect(cosmos.setPointPositions).toHaveBeenCalledExactlyOnceWith(expected, {
      dimensions: 3,
      dontRescale: true,
    });
  ...
  it.each([
    ...
    "fills missing $dimensions D nodes in the effective world with $existingCount existing node",
    ({ dimensions, existingCount }) => {
      vi.spyOn(console, "warn").mockImplementation(() => {});
      ...
      const result = Reflect.apply(fillMissingPointPositions, undefined, [
        cosmos,
        4,
        seed,
        dimensions,
      ...
      expect(result).toEqual([
        ...existing,
        ...getRandomPointPositions(4 - existingCount, 4096, seed, dimensions),
      ]);
