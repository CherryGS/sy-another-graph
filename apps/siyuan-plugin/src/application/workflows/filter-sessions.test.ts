...
const snapshot: SearchGraphSnapshot = {
  requestId: "first",
  label: "Topic",
  query: "topic",
...
describe("temporary search preset isolation", () => {
  ...
  it("preserves the original preset draft, lookup, and rule revision across temporary edits", () => {
    const session = new FilterSessions();
    session.setNormal((previous) => ({
      ...previous,
      query: "draft lookup",
      scopeId: snapshot.ids[0],
      hierarchy: true,
    }));
  ...
  it("stale preset acknowledgements do not leave a newer search", async () => {
    ...
    const pending = session.switchNormal(
      () =>
  ...
  it("rejects malformed or empty bridge snapshots", () => {
    ...
    expect(
      readSearchSnapshot({ ...snapshot, ids: [...snapshot.ids, ...snapshot.ids] })?.ids,
    ).toEqual(snapshot.ids);
  });
  it("resets search filters to connected all-type context without changing the original draft", () => {
    ...
    session.reset();
    expect(session.getSnapshot().temporary?.filters).toMatchObject({
      hierarchy: true,
      documentsOnly: false,
      scopeId: "",
    });
    ...
    session.leave();
    session.reset();
    expect(session.normalRef.current).toMatchObject({
      hierarchy: false,
      documentsOnly: true,
      notebook: "",
      query: "",
