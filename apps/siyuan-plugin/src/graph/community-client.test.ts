...
import { calculateCommunities, communityEndpoints, type CommunityWorkerPort } from "./community-client";
import type { PreparedGraph } from "./prepare-graph";

function worker(): CommunityWorkerPort {
  return { postMessage: vi.fn(), terminate: vi.fn(), onmessage: null, onerror: null, onmessageerror: null };
}
...
describe("community computation lifetime", () => {
  it("releases the worker after a validated result and excludes singleton groups from aggregation", async () => {
    ...
    w.onmessage!({ data: { membership: new Uint32Array([0, 0, 2, 3]), calculationMs: 2 } } as MessageEvent);
  ...
  it("terminates obsolete computation and rejects a late captured callback", async () => {
    const w = worker(), controller = new AbortController();
  ...
  it("rejects malformed membership instead of mapping it onto the displayed nodes", async () => {
    ...
    w.onmessage!({ data: { membership: new Uint32Array([0, 0, 9, 9]), calculationMs: 2 } } as MessageEvent);
  ...
  it("maps source indices through the current displayed order and rejects excluded endpoints", async () => {
    ...
    expect(await communityEndpoints(data, new AbortController().signal)).toEqual(new Uint32Array([1, 0]));
    await expect(communityEndpoints({ ...data, indexToNode: data.indexToNode.slice(0, 1) }, new AbortController().signal)).rejects.toThrow("端点");
  });
});
