...
describe("node colors", () => {
  it("assigns stable type colors across filtering, notebooks, and degree changes", () => {
    expect(new Set(Object.values(NODE_TYPE_COLORS)).size).toBe(
      Object.keys(NODE_TYPE_COLORS).length,
    );
    ...
    expect(nodeColor({ ...node, blockType: "p", notebook: "other", degree: 2000 }, "type")).toBe(
      NODE_TYPE_COLORS.p,
    );
    expect(nodeColor({ ...node, entity: "database", blockType: "p" }, "type")).toBe(
      NODE_TYPE_COLORS.database,
    );
    expect(nodeColor({ ...node, entity: "database-item" }, "type")).toBe(
      NODE_TYPE_COLORS["database-item"],
    );
  ...
  it("groups descendants by the first branch below their root", () => {
    ...
    expect(nodeColor({ ...node, path: "/root/concepts/nested/note.sy" }, "branch")).toBe(concept);
    expect(nodeColor({ ...node, path: "/root/projects.sy" }, "branch")).not.toBe(concept);
  ...
  it("preserves notebook colors and uses a bounded degree scale independent of visible nodes", () => {
    ...
    expect(nodeColor(node, "degree")).not.toBe(nodeColor({ ...node, degree: 30 }, "degree"));
  });
});
