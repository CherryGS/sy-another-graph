...
export function projectPosition(renderer: ProjectionApi, position: readonly number[]): Point2D | undefined {
  if (renderer.is3D)
    return renderer.spaceToScreenPosition([position[0], position[1], position[2] ?? 0], { dimensions: 3 });
...
export function cameraDepthOffset(camera: CameraState, position: readonly number[]): number {
  ...
  return (position[0] - camera.target[0]) * sinPolar * Math.sin(camera.azimuth)
