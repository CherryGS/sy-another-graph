...
export function useCommunities(data: PreparedGraph | null, enabled: boolean, resolution: number) {
  ...
  useEffect(() => {
    ...
    void communityEndpoints(data, controller.signal)
      .then((endpoints) =>
        calculateCommunities({ nodes: data.pointsCount, endpoints, resolution }, controller.signal),
      )
      .then((partition) => {
  ...
  const config: CosmographConfig = useMemo(
    () => ({
      ...
      pointClusterByFn: partition
        ? (index: number) =>
            partition.sizes[partition.membership[index]] > 1
  ...
  return {
    ...
    config,
    error: current?.error,
    pending: enabled && !!data?.pointsCount && !current,
  };
}
