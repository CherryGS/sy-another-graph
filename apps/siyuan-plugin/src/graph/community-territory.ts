...
const PALETTE = [[94, 170, 210], [179, 131, 204], [83, 184, 154], [211, 163, 92], [196, 113, 138], [128, 156, 218]];
...
export function territoryRaster(
  ...
  for (let i = 0; i < membership.length; i++) {
    ...
    const sx = (a * x + c * y + tx) / cell, sy = (b * x + d * y + ty) / cell;
    if (!Number.isFinite(sx) || !Number.isFinite(sy) || sx < 0 || sy < 0 || sx >= width || sy >= height) continue;
    const ix = Math.floor(sx), iy = Math.floor(sy), index = iy * width + ix;
