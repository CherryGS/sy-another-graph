...
describe("community territory geometry", () => {
  it("leaves distant empty space and singleton points unpainted", () => {
    const raster = territoryRaster(new Float32Array([30, 30, 36, 30, 540, 30]), new Uint32Array([0, 0, 2]), new Uint32Array([2, 0, 1]), new Map([[0, [100, 200, 50]]]), identity, 600, 120);
  ...
  it("keeps close communities distinct and bounds the raster on large displays", () => {
    const raster = territoryRaster(new Float32Array([30, 30, 36, 30, 60, 30, 66, 30]), new Uint32Array([0, 0, 2, 2]), new Uint32Array([2, 0, 2, 0]), new Map([[0, [255, 0, 0]], [2, [0, 255, 0]]]), identity, 120, 120);
    expect([...raster.pixels.slice((5 * raster.width + 5) * 4, (5 * raster.width + 5) * 4 + 3)]).toEqual([255, 0, 0]);
    expect([...raster.pixels.slice((5 * raster.width + 10) * 4, (5 * raster.width + 10) * 4 + 3)]).toEqual([0, 255, 0]);
    const large = territoryRaster(new Float32Array(), new Uint32Array(), new Uint32Array(), new Map(), identity, 7680, 4320);
