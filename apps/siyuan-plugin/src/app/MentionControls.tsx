    ...
    <FieldGroup className="gap-3">
      <Field>
        ...
        <ToggleGroup
          ...
          variant="outline"
          aria-label="文本提及模式"
          value={state.filters.mentions}
          className="w-full"
          onValueChange={(value) => {
            if (isMentionMode(value))
              state.setFilters((previous) => ({ ...previous, mentions: value }));
          }}
        >
      ...
      <MentionExclusions
        ...
        onApply={(excludedMentionPhrases) =>
          state.setFilters((previous) => ({ ...previous, excludedMentionPhrases }))
        }
      />
    </FieldGroup>
  );
