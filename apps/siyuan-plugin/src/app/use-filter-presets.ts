...
import { PresetClient } from "../presets/client";
import { PresetController, initialPresetSnapshot } from "../presets/controller";
import { applyPresetFilters, samePresetFilters } from "../presets/model";
...
export function useFilterPresets(
  ...
  useEffect(() => {
    const current = new PresetController(new PresetClient(window), {
      ...
      validate: rules => {
        ...
        if (!data) return rules.scopeId || rules.notebook ? "请等待图谱数据读取完成后切换预设。" : "";
  ...
  useEffect(() => { if (data) controller.current?.hydrate(); }, [data]);
  const active = snapshot.store.presets.find(preset => preset.id === snapshot.store.activePresetId);
  return {
    ...
    retry: () => { void controller.current?.load(); },
    ...
    copy: (id: string, name: string) => controller.current?.copy(id, name) ?? Promise.resolve(false),
    rename: (id: string, name: string) => controller.current?.rename(id, name) ?? Promise.resolve(false),
