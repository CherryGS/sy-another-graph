...
const wasm = initSync({ module: await readFile(new URL("../../wasm/graph_core_bg.wasm", import.meta.url)) });
for (const kind of ["sparse", "hub"]) {
  const nodes = 100_000;
  const endpoints = new Uint32Array(nodes * 10);
  let cursor = 0;
  for (let i = 0; i < nodes; i++) for (const skip of [1, 7, 97, 997, 7919]) {
    endpoints[cursor++] = i;
  ...
  console.log(JSON.stringify({ kind, nodes, inputPairs: endpoints.length / 2, groups, calculationMs, wasmMemoryBytes: wasm.memory.buffer.byteLength }));
