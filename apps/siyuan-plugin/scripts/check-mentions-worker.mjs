...
export async function verifyMentionWorker() {
  ...
  try {
    await new Promise((resolve, reject) => {
      timer = setTimeout(() => reject(new Error("Built mention worker did not complete a query")), 10_000);
      ...
      worker.on("message", message => {
        if (message.kind === "boot") {
          const blocks = [
            ...
            { id: "p", rootId: "a", type: "p", title: "", ial: "", markdown: '**A&amp;B** `A&B` ((20260822181032-6spbotb "A&B"))' },
          ];
          worker.postMessage({ kind: "load", revision: 1, blocks });
          worker.postMessage({ kind: "scope", revision: 1, scopeRevision: 1, scope: {
            entries: blocks.map((block, index) => ({ id: block.id, displayId: block.id, index })), explicitPairs: [],
          } });
          worker.postMessage({ kind: "query", revision: 1, scopeRevision: 1, request: 1, mode: "all", chosenIds: [] });
