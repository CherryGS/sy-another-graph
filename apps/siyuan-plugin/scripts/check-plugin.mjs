...
for (const file of ['index.js', 'index.css', manifest.readme.default]) {
  assert.ok(existsSync(new URL(`../dist/${file}`, import.meta.url)), `Missing build asset: ${file}`)
}
...
runInNewContext(bundle, {
  ...
  require(id) {
    assert.equal(id, 'siyuan', 'The empty scaffold should only require the host API')
    return { Plugin: HostPlugin }
  },
}, { filename: 'dist/index.js', timeout: 1_000 })

assert.equal(typeof pluginModule.exports, 'function', 'SiYuan expects a CommonJS plugin constructor')
assert.ok(pluginModule.exports.prototype instanceof HostPlugin, 'The entry must extend the host Plugin')
