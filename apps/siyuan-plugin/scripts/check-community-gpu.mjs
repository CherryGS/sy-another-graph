...
function run(shaders) {
  ...
  function target(t, size) {
    ...
    if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE)
      throw new Error("Incomplete float target");
    gl.viewport(0, 0, size, size);
    gl.clearColor(0, 0, 0, 0);
    gl.clear(gl.COLOR_BUFFER_BIT);
  }
  const positionTexture = texture(width, positions),
    clusterTexture = texture(width, assignments);
  const exitTexture = texture(width, exits),
    coefficient = texture(width, new Float32Array(positions.length).fill(1));
  const custom = texture(clusterWidth, new Float32Array(clusterWidth * clusterWidth * 4).fill(-1));
  const centermass = texture(clusterWidth, null),
    velocity = texture(width, null);
  ...
  for (const dimensions of [2, 3])
    for (const variant of ["installed", "edge-sampling-control"]) {
      const change = (text, forceShader = false) => {
        ...
        if (forceShader && variant === "edge-sampling-control")
          text = text.replaceAll(
            "(pointClusterIndices.xy + 0.5) / clustersTextureSize",
            "pointClusterIndices.xy / clustersTextureSize",
          );
        return dimensions === 3
          ? text.replace("#version 300 es", "#version 300 es\n#define SPACE_3D")
          : text;
      };
      const accumulate = program(change(shaders.di), shaders.li),
        force = program(quad, change(shaders.ci, true));
      target(centermass, clusterWidth);
      uniforms(
        accumulate,
        { pointsTextureSize: width, clustersTextureSize: clusterWidth },
        { positionsTexture: positionTexture, clusterTexture, exitTexture },
      );
      ...
      uniforms(
        force,
        { alpha: 0.3, clustersTextureSize: clusterWidth, clusterCoefficient: 0.3 },
        {
          positionsTexture: positionTexture,
          clusterTexture,
          centermassTexture: centermass,
          clusterPositionsTexture: custom,
          clusterForceCoefficient: coefficient,
        },
      );
      ...
      for (let i = 0; i < nodes; i++)
        for (let axis = 0; axis < dimensions; axis++) {
          const v = values[i * 4 + axis],
            sum = sums[membership[i]];
      ...
      results.push({
        dimensions,
        variant,
        pass: !invalid && maxError < 0.02 && !error,
        maxError,
        invalid,
        meanForce: net.map((v) => v / nodes),
        glError: error,
      });
  ...
  output.dataset.result = results.every((r) => (r.variant === "installed" ? r.pass : !r.pass))
    ? "pass"
    : "fail";
...
server.listen(0, "127.0.0.1", () =>
  console.log(`Community GPU regression: http://127.0.0.1:${server.address().port}/`),
);
