...
| 文档排除（ID / 标题文本 / 正则）    | 两组独立规则：仅文档内容保留子文档，或排除完整子树；合并后再投影，不受“包含子文档”开关缩减 | 是                                       |
...
[文档排除](../apps/siyuan-plugin/src/modules/content-exclusions/matcher.ts) 在独立 Worker 中匹配完整文档标题，
普通文本按包含匹配，正则使用固定 `iu` 标志；ID 也可指定原生块。每个标题只缓存两种范围是否命中，
不保存每个标题与所有规则的笛卡尔积。匹配根由与建图共享的 `excludedContentIds` 批量展开：
文档组包含自身及文内块，遇到子文档停止；子树组包含全部后代。两组按固定条目的顺序逐步累计排除，先限定源身份再投影。
分类文档自身的块、引用和提及均退出图，子文档自己的来源仍须通过其他范围条件；包含边不跨越被排除的祖先。

空文档是第三个固定条目，默认停用。读取时按所属文档汇总正文是否为空，标题、子文档和空白段落不算内容，未知类型保守保留。
连接保留使用前面排除已生效的投影图，忽略方向，以涉及空文档的边为 1、其他边为 0，计算稳定最小生成森林并剪掉空叶子。
只用森林选择节点，不替换原始边；数据库连接的原生锚点也作为保留端点，避免投影间接删除非空逻辑实体。
关闭连接保留时直接排除当前图中的空文档身份，子文档仍保留；后续排除仍可断开连接。

预览与已应用计算使用同一流程，未修改草稿时复用已应用结果，草稿变化时启动独立请求。
输入复制原生包含结构、完整标题、空状态和关系端点，不复制正文或引用证据。
每步按当前范围及类型投影后的节点统计新增移除，位于隐藏孤立节点之前；包含匹配引起的间接逻辑实体移除。
详情区分全部命中、此前移除、本步移除和保留的连接节点，每页最多显示 50 行。
源、规则替换或取消会终止旧 Worker，匹配超过 10 秒会失败。结果绑定准确源／规则身份，未完成或失败时不发布漏排除的图。
...
筛选栏的「分组」选择关闭、自动社区或自定义集合。自动社区仍分析当前计算图；
自定义集合以原生 ID 子树、完整文档标题／其他节点标签和正则取并集。
ID 沿未筛选源图的包含关系展开至全部后代，包括子文档及文内块；标题与正则仍独立匹配。
只向当前显示图中的节点分配成员，隐藏祖先不截断归属，也不恢复已排除或范围外节点。
按卡片顺序分配给第一个匹配的启用集合；仅标题和正则不向文内块或子文档传递归属。
未匹配节点保持无分组状态，不组成一个隐式的「其他」集合。

分组结果只进入聚拢和区域背景，不生成关系，不改变源范围、投影、邻域或最短路。
镜头、悬停和颜色变化不重新匹配；显示节点、规则、顺序或启用状态变化会替换匹配任务。
聚拢力度变化复用成员结果。切到自动社区或关闭时终止自定义匹配，反向切换也会取消自动社区计算。

预设 v2 独立保存分组方式、参数和集合定义；复制时完整复制。旧 v1 预设从浏览器原有的社区设置
得到各自独立的初始配置，下一次正常保存时写成 v2。浏览器保留一份迁移输入，避免尚未保存的新格式
在重载后丢失旧配置；v2 预设不再受它影响。临时搜索的分组与正常预设隔离。

## 保存与更新

普通筛选规则和显示孤立开关跟随预设；图内搜索词、选择、相机不属于预设。
文档排除的新增规则保存在 `exclusionRules`，每条含匹配种类、原值和范围；旧 `excludeIds` 继续表达 ID 子树排除。
读取旧预设时新增字段默认为空，两者合成编辑器的两组规则。草稿预览不改预设，点击应用后才改变当前筛选。
