...
文档组包含自身及文内块，遇到子文档停止；子树组包含全部后代。两组取并集，先限定源身份再投影。
分类文档自身的块、引用和提及均退出图，子文档自己的来源仍须通过其他范围条件；包含边不跨越被排除的祖先。

预览与已应用计算使用独立请求和同一匹配函数，输入只复制原生包含结构与完整文档标题，不复制正文或引用证据。
预览显示整个已读取工作区中的受影响原生身份，去重计数，详情只渲染当前页；它不统计可见图的边数或数据库实体。
...
按卡片顺序分配给第一个匹配的启用集合；文档不向文内块或子文档传递归属。
