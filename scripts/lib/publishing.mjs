...
export function planRelease(root, kind) {
  ...
  for (const file of files) {
    ...
    assert.equal(
      [...file.before.matchAll(pattern)].length,
      1,
      `无法唯一定位版本字段：${file.path}`,
    );
    file.after = file.before.replace(
      pattern,
  ...
  return {
    ...
    name: (manifest.displayName["zh-CN"] || manifest.displayName.default)
      .replace(/\s+/g, " ")
      .trim(),
  };
}
...
export function commandRunner(root, pnpm = process.env.npm_execpath) {
  return (command, args, { capture = false } = {}) => {
    ...
    if (result.error)
      throw new Error(`无法运行 ${command}：${result.error.message}`, { cause: result.error });
    if (result.status !== 0) {
      const error = new Error(
        `${command} ${args.join(" ")} 执行失败${result.stderr ? `\n${result.stderr.trim()}` : ""}`,
      );
...
function github(run, path, optional = false) {
  try {
    return JSON.parse(run("gh", ["api", path], { capture: true }));
  } catch (error) {
...
function preflight(plan, run) {
  ...
  for (const args of [
    ...
    assert.equal(
      remoteRepository(git(...args)),
      plan.repository.toLowerCase(),
      "origin 地址必须与 plugin.json 的 GitHub 仓库一致，且只有一个推送目标",
    );
  }
  ...
  assert.ok(
    !repo.private && !repo.archived && repo.permissions?.push,
    "需要一个可推送的公开 GitHub 仓库；请检查 gh 登录及仓库权限",
  );
  ...
  assert.equal(
    git("ls-remote", "--tags", "origin", `refs/tags/${plan.tag}`),
    "",
    `远端标签 ${plan.tag} 已存在`,
  );
  ...
  if (latest) {
    ...
    assert.ok(
      newerThan(plan.version, latest.tag_name.slice(1)),
      "新版本必须高于 GitHub Latest Release",
    );
...
function assertPrepared(plan, context, run) {
  assert.equal(
    ...
    context.head,
    "检查期间 HEAD 已变化，请重新准备发布",
  );
  assert.equal(
    run("git", ["diff", "--cached", "--name-only"], { capture: true }),
    "",
    "检查期间暂存区已变化，请先处理暂存区",
  );
  ...
  assert.deepEqual(
    changed,
    versionFiles.map((path) => `M ${path}`).sort(),
    "检查期间存在额外改动，请重新准备发布",
  );
...
function releaseNotes(plan, context, run, notesFile) {
  ...
  const subjects = run("git", ["log", "--reverse", "--format=%s", range], { capture: true })
    .split("\n")
    .filter(Boolean);
  ...
  return [
    ...
    ...(context.latest
      ? [
          "",
          `[完整变更](https://github.com/${plan.repository}/compare/${context.latest.tag_name}...${plan.tag})`,
        ]
      : []),
    "",
  ].join("\n");
...
export async function publishPlugin(
  plan,
  { run = commandRunner(plan.root), confirm, log = console.log, notesFile } = {},
) {
  ...
  try {
    ...
    const asset = join(recovery, "package.zip"),
      notesPath = join(recovery, "release-notes.md");
    ...
    assert.equal(
      git("rev-parse", `${releaseHead}^`),
      context.head,
      "版本提交的父提交发生变化，停止推送",
    );
    assert.deepEqual(
      git("diff-tree", "--no-commit-id", "--name-only", "-r", releaseHead).split("\n").sort(),
      [...versionFiles].sort(),
      "版本提交包含了额外文件，停止推送",
    );
    ...
    run("git", [
      ...
      "--atomic",
      "origin",
      `${releaseHead}:refs/heads/${context.branch}`,
      `refs/tags/${plan.tag}`,
    ]);
    run("gh", [
      "release",
    ...
    assert.ok(
      published.tag_name === plan.tag && !published.draft && !published.prerelease,
      "GitHub Latest Release 状态与目标版本不一致",
    );
    const attachment = published.assets?.find((item) => item.name === "package.zip");
    assert.ok(
      attachment?.state === "uploaded" && attachment.size === bytes.length,
      "GitHub 安装包尚未完整上传",
    );
    if (attachment.digest)
      assert.equal(attachment.digest, `sha256:${sha256}`, "GitHub 安装包摘要不匹配");
    ...
    if (!committed && git("rev-parse", "HEAD") === context.head) {
      for (const file of plan.files) {
        if (
          staged &&
          git("show", `:${file.path}`).replaceAll("\r\n", "\n").trim() ===
            file.after.replaceAll("\r\n", "\n").trim()
        )
          run("git", ["restore", "--staged", "--", file.path]);
        try {
          if (readFileSync(file.absolute, "utf8") === file.after)
            writeFileSync(file.absolute, file.before);
        } catch (error) {
...
async function question(prompt) {
  ...
  try {
    return await terminal.question(prompt, { signal: abort.signal });
  } finally {
    terminal.close();
  }
}
...
export async function runPublishCli(root, args) {
  ...
  if (!kind) {
    assert.ok(
      process.stdin.isTTY,
      "非交互终端请指定 --bump patch|minor|major，并使用 --dry-run 或 --yes",
    );
