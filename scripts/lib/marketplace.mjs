...
export function validateMarketplaceManifest(manifest, expectedPackage, tag) {
  ...
  if (tag !== undefined)
    assert.equal(tag, `v${manifest.version}`, "Release tag must match the plugin version");
  assert.ok(
    typeof manifest.author === "string" && manifest.author.trim(),
    "An author is required for the marketplace",
  );
  assert.match(
    manifest.url,
  ...
  assert.equal(
    new URL(manifest.url).pathname.split("/").at(-1),
    manifest.name,
    "Repository and plugin names must agree",
  );
  ...
  for (const field of ["displayName", "description", "readme"]) {
    ...
    assert.ok(
      ...
      `${field} must be a locale map`,
    );
    assert.ok(
      typeof locales.default === "string" && locales.default.trim(),
      `${field}.default is required`,
    );
    for (const [locale, value] of Object.entries(locales)) {
      if (locale !== "default")
        assert.equal(
          ...
          locale,
          `Use a canonical language tag: ${locale}`,
        );
      assert.ok(
        typeof value === "string" && value.trim(),
        `${field}.${locale} must be nonempty text`,
      );
...
export function packagePlugin({
  ...
  assert.ok(
    ...
    "Distribution must be an ordinary directory",
  );
  const manifest = JSON.parse(readFileSync(join(distribution, "plugin.json"), "utf8"));
  assert.deepEqual(
    manifest,
    expectedManifest,
    "Build output must match the current plugin manifest",
  );
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true }).sort((a, b) =>
      ...
      if (entry.isDirectory()) collect(absolute, `${path}/`);
        ...
        assert.ok(
          ...
          `Archive input must be an ordinary file: ${path}`,
        );
  ...
  try {
    ...
    assert.ok(
      ...
      "Archive output must be an ordinary file",
    );
  } catch (error) {
