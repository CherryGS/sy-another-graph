  ...
  for (const rows of Object.values(report)) {
    ...
    for (const row of rows) {
      for (const path of row.paths) {
        ...
        const source = typeof metadata.repository === "string" ? metadata.repository : metadata.repository?.url;
        const authors = typeof metadata.author === "string" ? metadata.author : metadata.author?.name;
        const lines = [key, "=".repeat(72), `Declared license: ${row.license}`,
          ...(authors ? [`Author: ${authors}`] : []), ...(source ? [`Source: ${source}`] : [])];
        ...
        for (const name of names) lines.push(`\n${name}\n${"-".repeat(72)}\n${readFileSync(join(path, name), "utf8").trim()}`);
        if (!names.length) lines.push("No standalone license file was included in this installed package; refer to its declared license and upstream source.");
        packages.set(key, lines.join("\n"));
      }
    }
  }
  assert.ok(packages.size, "The production dependency license report is empty");
  return ["Third-party production dependency notices", "",
    ...
    ...[...packages.entries()].sort(([a], [b]) => a.localeCompare(b, "en")).map(([, text]) => text), ""].join("\n\n");
}
...
export function collectRuntimeNotices(pluginDirectory) {
  ...
  const output = execFileSync(process.execPath, [pnpm, "--dir", pluginDirectory, "licenses", "list", "--prod", "--json"],
